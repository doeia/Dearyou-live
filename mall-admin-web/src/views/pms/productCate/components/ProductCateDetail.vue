  ...
  <el-card class="form-container" shadow="never">
    <el-form :model="productCate"
      ...
      <el-form-item label="上级分类：">
        ...
        <!--        </el-select>-->
        <el-cascader
          v-model="productCateSelectedValue"
          placeholder="请选择分类"
          :options="productCateTreeList"
          change-on-select
      ...
      <el-form-item label="专区大图：">
        <single-upload v-model="productCate.bigPic"></single-upload>
      </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      async fetchListWithChildren() {
        const response = await fetchListWithChildren()
        this.productCateTreeList = response.data
        this.productCateTreeList.unshift({id: 0, name: '无上级分类'})
        this.productCateSelectedValue = [this.productCate.parentId]
      },
          ...
          if (valid) {
            this.$confirm('是否提交数据', '提示', {
              ...
              if (this.isEdit) {
                ...
                this.productCate.parentId = this.productCateSelectedValue[this.productCateSelectedValue.length - 1]
