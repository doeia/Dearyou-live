...
public class UmsPermissionServiceImpl implements UmsPermissionService {
    ...
    public int create(UmsPermission permission) {
        ...
        permission.setSort(permission.getSort()==null? 0: permission.getSort());
        if(StringUtils.isEmpty(permission.getValue())){
            //TODO value不允许为空字符串，否则spring security会抛异常
            permission.setValue(null);
        }
        if(permission.getPid()==null){
            permission.setPid(0L);
        }else if(permission.getPid()!=0 && null == permissionMapper.selectByPrimaryKey(permission.getPid())){
            return 0;
        }
